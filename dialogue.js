...
function handleRoomDialogue() {
 ...
 if (currentDialogueIndex === 0) {
    if (dialogueLines[0] === "< 안에 무언가가 들어있을 것만 같은 느낌이 든다. >") {
      showPotOptions = true; 
      currentDialogueIndex = -1; // 대사창 상태 초기화 (멈춤 방지)
      revealedLength = 0;
      return; 
    }
    if (dialogueLines[0] === "..낡은 문이다.") {
      showDoorOptions = true; 
      currentDialogueIndex = -1; // 대사창 상태 초기화 (멈춤 방지)
      revealedLength = 0;
      return; 
    }
    if (dialogueLines[0] === "< 동상의 입에 쪽지가 있다. 꺼내야 할 것 같다. >") {
